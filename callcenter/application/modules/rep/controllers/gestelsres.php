...
class Gestelsres extends MX_Controller
{
	...
	function index()
	{
		   ...
		   $this->load->view('rep/gestelres_view');
	...
	function generar_pdfTel($id_empleado,$fecha_ini,$fecha_fin,$empleado){
		...
		if ($id_empleado=="0"){
			$result = new Gestel();
			$query=$result->resumen_llamadas_contactos($fecha_inicial,$fecha_final);
		...
		//$gestor_dato;
			...
			$result = new Gestel();
			$query=$result->resumen_llamadas_contactos_empleados($id_empleado,$fecha_inicial,$fecha_final);
			$obj=$query->result_array();
...
// datos por defecto de cabecera, se pueden modificar en el archivo tcpdf_config_alt.php de libraries/config
        $pdf->SetHeaderData('logo.jpg', PDF_HEADER_LOGO_WIDTH, 'RESUMEN DE GESTION TELEFONICA ', 'DESDE: '.$fecha_ini.' HASTA: '.$fecha_fin, array(0, 64, 255), array(0, 64, 128));
...
//fijar efecto de sombra en el texto
        ...
        $group_gestor = "";
		/*
		foreach ($obj as $clave => $fila){
			 $gestor[$clave] = $fila['gestor'];
		}
		*/
		//sort($obj);
	    foreach ($obj as $fila)
        {
			...
			if ($group_gestor != $gestor)
            {
                           
              //$html .= "<tr><td>".$gestor."</td></tr>";
            }
            
            if (!empty($gestor)){
			$html .= "<tr><td>".$fecha_llamada."</td>
			              <td>".$gestor."</td>
			              <td>".$nombres."</td><td>".$apellidos."</td><td>".$telefono."</td>
