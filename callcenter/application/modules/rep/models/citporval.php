...
class Citporval extends DataMapper {
    ...
    function __construct($id = NULL)
    {
		...
		function citas_contactos($fecha_inicial = NULL,$fecha_final = NULL){
			$sql=" SELECT CONCAT(emp.nombres,' ',emp.apellidos) as teleoperador,con.identificacion,
				con.nombres,con.apellidos,(select telefono from llamadas where id=ci.llamada_id) as telefono,
				...
				WHERE
                cia.estado='AC' and ci.cita_estado='Nueva'
                and not exists (select * from citas where padre_id=ci.id) 
