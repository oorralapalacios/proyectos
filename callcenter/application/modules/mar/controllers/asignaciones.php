...
class Asignaciones extends MX_Controller{
    ...
    function index(){
		...
		$this->load->view('mar/asignacion_view');
		...
		$this->load->view('mar/asignacion_selasignar');
		$this->load->view('mar/asignacion_selreasignar');
	...
	function datos($estado){
		...
		$obj = new Asignacion();
		$data=$obj->bandejafilter($estado);
		return $data;

	}
	
	function datosgen($estado){
		//$accion;
		
		$obj = new Contacto_subido();
		$data=$obj->bandejafilter(null,$estado);
		return $data;
   ...
   function horaesmenorigual($hora1,$hora2){
    ...
    function ajax(){
        ...
        switch($this->input->post('accion')){
			case 'dataup':
				   echo json_encode($this->datos('AC'));
				break;
