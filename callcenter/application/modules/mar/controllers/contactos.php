...
class Contactos extends MX_Controller{
	...
	function datos($camp_id, $emp_id, $bandeja){
		$obj = new Contacto_Campana();
		$data=$obj->bandejafilter($camp_id, $emp_id,$bandeja);
		return $data;	
	}
		
	function datosequivocados($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->equivocados($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	function datosnotitulares($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->notitulares($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	
	function datosnointeresados($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->nointeresados($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	
	function datosfueracobertura($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->fueracobertura($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	function seguimiento($camp_id, $emp_id){
		$obj = new Seguimiento_Telefonico();
		$query=$obj->custom_query($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	function seguimientoinbound($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->custom_query($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	...
	function datospagodirecto($camp_id, $emp_id){
		$obj = new Contacto_Campana();
		$query=$obj->pagodirecto($camp_id, $emp_id);
		$results=$query->result_array();
		echo json_encode($results);	
	}
	...
	function llamadasfiltradaspaginadas($conta_id){
	     ...
	     $obj = new Llamada();
		 $data=$obj->pageContactoLlamadas($conta_id);
		 echo json_encode($data);
		
	}
	
	function campanasfiltradaspaginadas($conta_id){
				 
	     $obj = new Contacto_Campana();
		 $data=$obj->pageContatoCampanas($conta_id);
		 echo json_encode($data);
		
	}
	
	function citasfiltradaspaginadas($conta_id){
				 
	     $obj = new Cita();
		 $data=$obj->pageContatoCitas($conta_id);
		 echo json_encode($data);
    ...
    function ajax(){
        ...
        switch($this->input->post('accion')){
			case 'dataGC'://Contactos asignados
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'GC'));
				 break;
		    case 'dataST': //Seguimiento telefónico
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'ST'));
				 break;
		    case 'dataIN': //Seguimiento inbound
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'IN'));
				 break;
			case 'dataSTA': //Seguimiento telefónico asignado
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'STA'));
				 break;
		    case 'dataSTR': //Seguimiento telefónico gestionado
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'STR'));
				 break;		 
		    case 'dataEQ'://Equivocadas
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'EQ'));
				 break;
		    case 'dataNT'://No titulares
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'NT'));
				 break;
			case 'dataNI'://No Interesados
				 echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'NI'));
				 break;
		    case 'dataFC'://Fuera de cobertura
		         echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'FC'));
				 break;
			case 'dataPD'://Pago de directo
		         echo json_encode($this->datos($this->input->post('camp_id'),$this->input->post('emp_id'),'PD'));
				 break;
			case 'record':
				 $obj=new Contacto();
			...
			case 'addllamada':
                    $obj = new Llamada();
				    $obj->trans_begin();
					$obj->contacto_campana_id=$this->input->post('contacto_campana_id');
            ...
            case 'segtel':
					...
					$obj7->empleado_id=$this->input->post('empleado_id');
					$obj7->campana_id=$this->input->post('campana_id');
					$obj7->contacto_id=$this->input->post('contacto_id');
					$obj7->telefono=$this->input->post('telefono');
					//$obj7->fecha_hora=date("Y-m-d H:i:s",strtotime ($this->configFechaLlamada($emp_id)));
					$obj7->fecha_hora=$this->input->post('fecha_hora');
					$obj7->observaciones=$this->input->post('observacion');
					$obj7->proceso='Reasignado';
					$obj7->bandeja='ST';
					$obj7->padre_id=$this->input->post('contacto_campana_id');
            ...
            case 'addcita':	
				    ...
				    $cit=new Cita();
					$cit->tipo_gestion=$this->input->post('tipo_gestion');
					$cit->contacto_campana_id=$this->input->post('contacto_campana_id');
					...
					}else if($this->input->post('limite_credito')==0){
						$cit->limite_credito=0;	
					}
				    if ($this->input->post('financiamiento')){
				      $cit->financiamiento=$this->input->post('financiamiento');	
				    }else if($this->input->post('financiamiento')==0){
						$cit->financiamiento=0;	
					}
					if ($this->input->post('limite_credito_tc')){
					  $cit->limite_credito_tc=$this->input->post('limite_credito_tc');	
					}else if($this->input->post('limite_credito_tc')==0){
						$cit->limite_credito_tc=0;	
					}
				    if ($this->input->post('financiamiento_tc')){
				      $cit->financiamiento_tc=$this->input->post('financiamiento_tc');	
				    }else if($this->input->post('financiamiento_tc')==0){
						$cit->financiamiento_tc=0;	
					}
					foreach ($this->input->post('agenda') as $arr) {
				 		$cit->fecha_hora=$arr['start'];
					}
					...
					if ($cit->id){
					 ...
					 foreach ($this->input->post('encuesta') as $arr) {
					 	        $contacto_campana_id=$this->input->post('contacto_campana_id');
								...
								if ($id!=0){	
									if ($this->input->post('multi')==1){
										...
										$where_m = "contacto_campana_id = ".$contacto_campana_id." AND preguntas_id = ".$pregunta_id." AND opciones_respuesta= '".$arr['resp']."'";
										$res->where($where_m)->get();
										$res->opciones_respuesta=$arr['resp'];//cambiar a varchar
										$res->contacto_campana_id=$contacto_campana_id;
					...
					//Finaliza LLamada Visista in situ
					 ...
					 $cal = new Llamada();
					 $cal->contacto_campana_id=$this->input->post('contacto_campana_id');
