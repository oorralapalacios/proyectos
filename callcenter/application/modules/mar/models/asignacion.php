...
class Asignacion extends DataMapper {
	...
	$start=$pagenum*$pagesize;
            ...
            $sql="select SQL_CALC_FOUND_ROWS co.*,
             (select nombre from campanas ca where ca.id=co.campana_id) as campana,
             (select concat(e.apellidos,' ',e.nombres) from contactos_campanas cc, contactos c, empleados e
              where cc.contacto_id=c.id and cc.empleado_id=e.id
			  and cc.campana_id=co.campana_id
			  and cc.telefono=co.telefono_movil
			  and c.identificacion=co.identificacion
			  and cc.estado='AC'
			  order by cc.fecha_ing desc limit 1) as ultimo_teleoperador_asignado,
              (select cc.fecha_ing from contactos_campanas cc, contactos c, empleados e
              where cc.contacto_id=c.id and cc.empleado_id=e.id
			  and cc.campana_id=co.campana_id
			  and cc.telefono=co.telefono_movil
			  and c.identificacion=co.identificacion
			  and cc.estado='AC'
			  order by cc.fecha_ing desc limit 1) as fecha_ultima_teleoperacion,
			  (select cc.id from contactos_campanas cc, contactos c, empleados e
              where cc.contacto_id=c.id and cc.empleado_id=e.id
			  and cc.campana_id=co.campana_id
			  and cc.telefono=co.telefono_movil
			  and c.identificacion=co.identificacion
			  and cc.estado='AC'
			  order by cc.fecha_ing desc limit 1) as padre_id,
			  (select cc.contacto_id from contactos_campanas cc, contactos c, empleados e
              where cc.contacto_id=c.id and cc.empleado_id=e.id
			  and cc.campana_id=co.campana_id
			  and cc.telefono=co.telefono_movil
			  and c.identificacion=co.identificacion
			  and cc.estado='AC'
			  order by cc.fecha_ing desc limit 1) as contacto_id,
			  (select respuesta_recibida from llamadas ll, contactos c
              where ll.contacto_id = c.id
              and ll.campana_id=co.campana_id 
              and ll.telefono = co.telefono_movil
              and c.identificacion=co.identificacion
			  order by ll.fecha_ing desc limit 1) as estado_ultima_llamada,
              (select ll.fecha_ing from llamadas ll, contactos c
              where ll.contacto_id = c.id
              and ll.campana_id=co.campana_id 
              and ll.telefono = co.telefono_movil
              and c.identificacion=co.identificacion
			  order by ll.fecha_ing desc limit 1) as fecha_ultima_llamada,
			 (select concat(e.apellidos,' ',e.nombres) from llamadas ll, contactos c, empleados e
              where ll.contacto_id = c.id
              and ll.empleado_id=e.id
              and ll.campana_id=co.campana_id 
              and ll.telefono = co.telefono_movil
              and c.identificacion=co.identificacion
			  order by ll.fecha_ing desc limit 1) as empleado_ultima_llamada,
			  (select cita_estado from citas ci, contactos c, empleados e, llamadas ll 
              where ci.contacto_id=c.id
              and ci.empleado_id=e.id
              and ci.campana_id=co.campana_id
              and ci.llamada_id=ll.id
			  and ll.telefono = co.telefono_movil
              and c.identificacion=co.identificacion
              order by ci.fecha_ing desc limit 1) as estado_ultima_cita,
              (select ci.fecha_ing from citas ci, contactos c, empleados e, llamadas ll 
              where ci.contacto_id=c.id
              and ci.empleado_id=e.id
              and ci.campana_id=co.campana_id
              and ci.llamada_id=ll.id
			  and ll.telefono = co.telefono_movil
              and c.identificacion=co.identificacion
              order by ci.fecha_ing desc limit 1) as fecha_ultima_cita
 			from contactos_subidos co
	...
	if (isset($_POST['filterscount']))
	{
		...
		if ($filterscount > 0)
		{
			...
			for ($i=0; $i < $filterscount; $i++)
		    {
				...
				{
					case "CONTAINS":
						if ($filterdatafield = "telefono_movil"){
							$where .= " identificacion in (select identificacion from contactos_subidos where telefono_movil like '%" . $filtervalue. "%')";
					...
					case "EQUAL":
						if ($filterdatafield = "telefono_movil"){
							$where .= " identificacion in (select identificacion from contactos_subidos where telefono_movil in (" . $filtervalue .")) ";
						}
						else {
