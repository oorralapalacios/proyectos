...
class Estasi extends DataMapper {
	...
	function llamadas_contactos($fecha_inicial = NULL,$fecha_final = NULL){
		...
		function historia_llamadas_contactos($fecha_inicial,$fecha_final){
			$sql="/*Asignaciones y reasignaciones*/
			  select DATE_FORMAT(cc.fecha_hora,'%Y/%m/%d') as fecha, CONCAT(co.apellidos,' ',co.nombres) as cliente,CONCAT(em.nombres,' ',em.apellidos) As gestor, cc.telefono, proceso,
							'0' as duracion, 'Planificada' as estado,
							 CASE bandeja when 'GC' then CONCAT('bandeja de ','gestión de contactos') 
							 	when 'ST' then CONCAT('bandeja de ', 'seguimiento telefónico')
							    END as respuesta, ca.nombre as campana, 1 as total
							from contactos_campanas cc, contactos co, empleados em, campanas ca
							where cc.contacto_id=co.id
							and cc.empleado_id=em.id
							and cc.campana_id=ca.id
			                and cc.fecha_hora between  '".$fecha_inicial."' AND '".$fecha_final."'
			  /*Reasignaciones salientes*/
			  union all
			  select DATE_FORMAT(ch.fecha_hora,'%Y/%m/%d') as fecha, CONCAT(co.apellidos,' ',co.nombres) as cliente,CONCAT(em.nombres,' ',em.apellidos) As gestor, cc.telefono, ch.proceso,
							'0' as duracion, 'Planificada' as estado,
							 CASE cc.bandeja when 'GC' then CONCAT('bandeja de ','gestión de contactos') 
							 	when 'ST' then CONCAT('bandeja de ', 'seguimiento telefónico')
							    END as respuesta, ca.nombre as campana, -1 as total
							from contactos_campanas cc, contactos co, empleados em, campanas ca, contactos_campanas ch
							where ch.padre_id=cc.id 
			                and cc.contacto_id=co.id
							and cc.empleado_id=em.id
							and cc.campana_id=ca.id
			                and ch.padre_id is not null
			                and ch.fecha_hora between  '".$fecha_inicial."' AND '".$fecha_final."'
				;";
				//$params = array($fecha_inicial,$fecha_final);
				//$query=$this->db->query($sql,$params);
				$query=$this->db->query($sql);
